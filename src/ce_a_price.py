...
def CDS_price_CEa(rec_rate, maturities, params_cir, params_ce, params_gbm):
    ...
    d1 = lambda s : (np.log(x_ratio) + (alpha - 0.5 * sigma_x) * s
          ) / (sigma_x * np.sqrt(s))
    d2 = lambda s : (- np.log(x_ratio) + (alpha - 0.5 * sigma_x) * s
          ) / (sigma_x * np.sqrt(s))
    protection_leg = ( (1 - 
                        (ndtr(d1(maturities)) - 
...
if __name__ == "__main__":
    ...
    protection_leg, premium_leg, price  = CDS_price_CEa(0.4, t_grid, params_cir, params_ce, params_gbm)
